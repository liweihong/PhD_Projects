...
# global settings
#  [-85.459274 - 10.192511], Y : [-3.687565 - 61.314644], Z : [0.077313 - 100.837891]
my $sample_num=1000;
my $root_dir="result/point_cloud_SH_$sample_num";
my $dataset="../data/pts/ShepardHall_trans.ptx";
#my $dataset="result/LinesOnly/ShepardHall_trans.ptx";
my $im_w=1024;
my $im_h=1024;
...
my $z_min =  -5.07;
my $z_max = 105.83;
# -x -85.45 -X 10.19 -y -8.68 -Y 61.31 -z -5.07 -Z 105.83
...
#    system $cmd_ir_ex;
    ...
    # generate iv from IR
#    my $cmd_iv="project -j -O \"-p $root_dir/ir.txt -P $root_dir/model.iv -t 7 -w $im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 1\" >> $log_ir_ex";
   my $cmd_iv="project -j -O \"-p $result_seg_path_dir/seg_ir.txt -P $result_seg_path_dir/seg_ir_$ksd_GC_thres.iv -t 7 -w $im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 1\" >> $log_ir_ex";
