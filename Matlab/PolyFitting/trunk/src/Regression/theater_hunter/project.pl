...
#my $root_dir="result/point_cloud_process_$sample_num";
my $root_dir="result/point_cloud_interior_$sample_num";
if ( ! -e $root_dir ) {
    mkdir $root_dir;
}
...
my $not_ignore_step_5=1;  # BPA
my $not_ignore_step_6=1;  # model generation without taper
my $not_ignore_step_7=1;  # error computaton
...
if ( $not_ignore_step_3 && ( ! -e $log_recover || -z $log_recover)) {
	...
	my $cmd_mv = "cp -r $clear_path_dir $reco_path_dir";
	print "$cmd_mv\n";
	system $cmd_mv;
    }
}
...
#    my $cmd_bpa="project.exe -S -O \"-a 0 -A $sample_num -f -P $result_path_dir -l $bpa_gap_length -m $bpa_ignore_multi_bnds -o $bpa_starting_dir -p 1 -r $bpa_radius_size -R -L $bpa_radius_min -H -I $bpa_reshape_len -E -C $bpa_centroid_diff\" > $log_bpa";
    my $cmd_bpa="project -L param.ini -S -O \"-a 0 -A $sample_num -P $result_path_dir\" > $log_bpa";
    print "$cmd_bpa\n";
    system $cmd_bpa;
}
$end_time = strftime "%H:%M:%S", localtime;
...
if ( $not_ignore_step_6 && (! -e $log_ir_ex || -z $log_ir_ex) ) {
    ...
    # generate iv from IR
    my $cmd_iv="project -j -O \"-p $root_dir/ir.txt -P $root_dir/model.iv -t 7 -w 640 -M 1\" >> $log_ir_ex";
    print "$cmd_iv\n";
    system $cmd_iv;
}
