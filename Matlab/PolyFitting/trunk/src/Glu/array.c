...
array_join(array_t *array1, array_t *array2)
{
	...
	fprintf(stderr, "array: join not defined for arrays of different sizes\n");
	/* NOTREACHED */
    }
    array = ALLOC(array_t, 1);
    if (array == NIL(array_t)) {
...
void
array_sort3(array_t *array, int (*compare)(const void *, const void *))
{
...
array_abort(array_t *a, int i)
{
    ...
    fprintf(stderr, "array package error");
