/*
 * $Id: avl.c,v 1.2 2004/07/20 20:49:38 ilya Exp $
 *
 */
/* LINTLIBRARY */
...
static int do_check_tree(avl_node *node, int (*compar)(const char *, const char *), int *error);

avl_tree *
#ifndef ILYA
avl_init_table(int (*compar)(const char *, const char *))
#else
avl_init_table2(int (*compar)(const char *, const char *))
#endif
{
    avl_tree *tree;
...
avl_lookup(avl_tree *tree, char *key, char **value_p)
{
    ...
    register int (*compare)(const char *, const char *) = tree->compar, diff;
...
avl_insert(avl_tree *tree, char *key, char *value)
{
    ...
    register int (*compare)(const char *, const char *) = tree->compar;
...
avl_delete(avl_tree *tree, char **key_p, char **value_p)
{
    ...
    register int stack_n = 0;
    char *key = *key_p;
    int (*compare)(const char *, const char *) = tree->compar, diff;
    avl_node **stack_nodep[STACK_SIZE];
...
static void 
avl_walk_forward(avl_node *node, void (*func)(const char *, const char *))
{
    if (node != NIL(avl_node)) {
...
static void 
avl_walk_backward(avl_node *node, void (*func)(const char *, const char *))
{
    if (node != NIL(avl_node)) {
...
int 
avl_numcmp(const char *x, const char *y)
{
