...
# global settings
my $sample_num=1000;
my $root_dir="result/point_cloud_interior_GCT_$sample_num";
my $dataset="../data/pts/GCT_trans.ptx";
my $im_w=640;
my $im_h=800;
...
my $z_min = -43.68;
my $z_max =   5.69;
# -x -15.36 -X 80.23 -y -30.18 -Y 89.93 -z -43.68 -Z 5.69

my $log_load="log_1_2_load.txt";
if ( $not_ignore_step_1 && ( ! -e $log_load || -z $log_load)) {
    my $cmd_load_point_cloud="project -l $dataset -O \"-s $sample_num -p $load_path_dir -w $im_w -h $im_h -d 1 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_load";
...
if ( $not_ignore_step_1 && ( ! -e $log_ksd_prep_load || -z $log_ksd_prep_load)) {
    my $cmd_prep_load_point_cloud="project -l $dataset -O \"-s $lr_sample_num -p $load_lr_path_dir -w $lr_im_w -h $lr_im_h -d 0 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_ksd_prep_load";
...
if ( $not_ignore_step_1 && ( ! -e $log_taper_load || -z $log_taper_load)) {
    my $cmd_taper_load="project -l $dataset -O \"-s $fi_sample_num -p $load_fi_path_dir -w $fi_im_w -h $fi_im_h -d 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_taper_load";
...
if ( $compute_direction == 0 ) { # bottom-up
...
}  elsif ( $compute_direction == 1 ) { # left-right

    $clear_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_right_clear";

    if ( $not_ignore_step_2 && ( ! -e $log_noise || -z $log_noise)) {
	if ( ! -e $clear_path_dir ) {
	    mkdir $clear_path_dir ;
	}
	my $cmd_clear="project -M -O \"-s 0 -e $sample_num -r $load_lr_path_dir/image_slice -w $clear_path_dir/image_slice -m $load_lr_path_dir\_masks\" > $log_noise";
	print "$cmd_clear\n";
	system $cmd_clear;
    }
}
...
if ( $compute_direction == 0 ) { # bottom-up
    ...
    $reco_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_right_recovered";
	...
	my $cmd_recover="project -O \"-s 0 -e $sample_num -p $reco_path_dir -P $load_lr_path_dir\" -1 > $log_recover";
	print "$cmd_recover\n";
	system $cmd_recover;
    } else {
	if ( $not_ignore_step_2 ) {
	    my $cmd_del = "$del_dir $clear_path_dir";
	    print "$cmd_del\n";
	    system $cmd_del;
	}
...
# this is for copying ir_vertical.txt to here.
my $result_seg_path_dir0="$root_dir/slices_$sample_num\_$im_w\_$im_h\_bottom_up_seg_result_$sample_num\_$ksd_HD_thres\_$bpa_radius_min";
if ( ! -e $result_seg_path_dir0 ) {
    mkdir $result_seg_path_dir0 ;
}

my $result_seg_path_dir;
if ( $compute_direction == 0 ) { # bottom-up
    ...
    $result_seg_path_dir="$root_dir/slices_$sample_num\_$im_w\_$im_h\_bottom_up_seg_result_$sample_num\_$ksd_HD_thres\_$bpa_radius_min";

    my $log_ksd="log_4_key_slice.txt";
    my $ksd_ref=321;
    #my $ksd_ref=429;
    #my $ksd_ref=124;
    my $ksd_end=880; # after 578, only two end face should be kept.
	...
	my $cmd_ksd="project -K -O \"-L $ksd_ref -R $ksd_end -P $reco_path_dir/image_slice -O $result_seg_path_dir -f seg_ir.txt -A 1 -s 1000 -b 2 -B 0.5 -c 2\" > $log_ksd";
	print "$cmd_ksd\n";
	system $cmd_ksd;
    }
    ...
    $bnd_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_right_boundary";
    if ( ! -e $bnd_path_dir ) {
	mkdir $bnd_path_dir ;
    }
    $result_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_right_result_$sample_num\_$ksd_HD_thres\_$bpa_radius_min";
    if ( ! -e $result_path_dir ) {
	mkdir $result_path_dir ;
    }

    $result_seg_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_reight_seg_result_$sample_num\_$ksd_HD_thres\_$bpa_radius_min";
    ...
    my $ksd_end=811;
    #my $ksd_ref=250;
    #my $ksd_end=350; # after 578, only two end face should be kept.
    my $ksd_path_dir="$root_dir/slices_$sample_num\_$lr_im_w\_$lr_im_h\_left_right";
    if ( $not_ignore_step_4 && (! -e $log_ksd || -z $log_ksd)) {
	#my $cmd_ksd="project -K -O \"-a 0 -d $ksd_HD_thres -r $ksd_ref -R $ksd_end -P $ksd_path_dir -O $result_path_dir\" > $log_ksd";
	...
	# copy seg_ir.txt ir_vertical.txt
	my $cmd_mv = "$cop $result_seg_path_dir/seg_ir.txt $result_seg_path_dir0/ir_vertical.txt";
	$cmd_mv =~ s/\//\\/g;
	print "$cmd_mv\n";
	system $cmd_mv;
    }
...
if ( $compute_direction == 0 ) { # bottom-up
	...
	#my $cmd_ir_ex="project -E -L param.ini -O \"-a 0 -s 0 -e $sample_num -p $result_path_dir\" > $log_ir_ex";
	#print "$cmd_ir_ex\n";
	#system $cmd_ir_ex;

	# generate iv from IR
	# let do this in the second round.
	#my $cmd_iv="project -j -O \"-p $root_dir/ir.txt -P $root_dir/model.iv -t 11 -0 1 -1 0 -w $im_w -W $lr_im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 0 -f\" >> $log_ir_ex";
	my $cmd_iv="project -j -O \"-p $result_seg_path_dir/seg_ir.txt -P $result_seg_path_dir/seg_ir_$ksd_GC_thres.iv -t 11 -0 1 -1 0 -w $im_w -W $lr_im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 0 -f\" ";
	print "$cmd_iv\n";
	system $cmd_iv;
    }
} elsif ( $compute_direction == 1 ) { # left_right

    if ( 0 && $not_ignore_step_6 && (! -e $log_ir_ex || -z $log_ir_ex) ) {
	my $cmd_ir_ex="project -E -L param.ini -O \"-a 0 -s 0 -e $sample_num -p $result_path_dir -f ir_vertical.txt\" > $log_ir_ex";
	print "$cmd_ir_ex\n";
	system $cmd_ir_ex;
...
if ( $need_copy == 1) {
	...
	system ($cop, "$root_dir\\ir_vertical.txt", "$root_dir\\ir_vertical_$sample_num\_$ksd_HD_thres\_$bpa_radius_min\.txt");
...
#    }
    clear_sub_face;
#    proj_sub 1, $i, 1, 0.2;  # argv[0]: BPA radius | argv[1]: HD threshold | left_right | Graph Cut 
#    clear_sub_face;
    proj_sub 1, $i, 0, 0.2;  # argv[0]: BPA radius | argv[1]: HD threshold | bottom_up  | Graph Cut
}
