...
# global settings
#X : [-21.897758 - 41.574398], Y : [-2.493792 - 17.405823], Z : [-4.382273 - 28.938585]
my $sample_num=1000;
my $root_dir="result/point_cloud_GH_$sample_num";
my $dataset="../data/pts/GreatHall_trans.ptx";
my $im_w=1024;
my $im_h=512;
...
my $z_min =  -7.38;
my $z_max =  28.93;
# -x -25.89 -X 41.57 -y -5.49 -Y 18.40 -z -7.38 -Z 28.93

my $log_load="log_1_2_load.txt";
if ( $not_ignore_step_1 && ( ! -e $log_load || -z $log_load)) {
    my $cmd_load_point_cloud="project -l $dataset -O \"-s $sample_num -p $load_path_dir -w $im_w -h $im_h -d 0 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_load";
...
if ( $not_ignore_step_1 && ( ! -e $log_ksd_prep_load || -z $log_ksd_prep_load)) {
    my $cmd_prep_load_point_cloud="project -l $dataset -O \"-s $lr_sample_num -p $load_lr_path_dir -w $lr_im_w -h $lr_im_h -d 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_ksd_prep_load";
...
if ( $not_ignore_step_1 && ( ! -e $log_taper_load || -z $log_taper_load)) {
    my $cmd_taper_load="project -l $dataset -O \"-s $fi_sample_num -p $load_fi_path_dir -w $fi_im_w -h $fi_im_h -d 1 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -t 1\" > $log_taper_load";
...
if ( $compute_direction == 0 ) { # bottom-up
    ...
    my $ksd_ref=156;
    #my $ksd_end=698; # after 578, only two end face should be kept.
    my $ksd_end=960; # after 578, only two end face should be kept.
...
if ( $compute_direction == 0 ) { # bottom-up
	...
	#my $cmd_iv="project -j -O \"-p $root_dir/ir.txt -P $root_dir/model.iv -t 11 -0 0 -1 2 -w $im_w -W $lr_im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 0 -f\" >> $log_ir_ex";
	my $cmd_iv="project -j -O \"-p $result_seg_path_dir/seg_ir.txt -P $result_seg_path_dir/seg_ir_$ksd_GC_thres.iv -t 11 -0 0 -1 2 -w $im_w -W $lr_im_w -M 2 -x $x_min -X $x_max -y $y_min -Y $y_max -z $z_min -Z $z_max -D 0 -f\" ";
	print "$cmd_iv\n";
	system $cmd_iv;
    }
...
#    }
    clear_sub_face;
    proj_sub 1, $i, 1, 0.2;  # argv[0]: BPA radius | argv[1]: HD threshold | left_right
    clear_sub_face;
    proj_sub 1, $i, 0, 0.2;  # argv[0]: BPA radius | argv[1]: HD threshold | bottom_up
}
