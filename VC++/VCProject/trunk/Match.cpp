...
const int NAVI_SIZE = 15; // region for searching. => an important parameter for final match.
const int VALID_P = 15;   // connected component size thres. 
const double MATCHED_RATIO = 0.2; // matched ratio.
const int LEN = 120;
...
void matching(BYTE **ppt_image, BYTE **mimio_image, Bitmap *ppt_bitmap, Bitmap *mimio_bitmap, Bitmap *&matching, Bitmap *&matched, double *matrix_val)
{
	...
	if (good_matched_num < 4)
	{
		cout<<"Good Matched points are " << good_matched_num << ", which is less than 4 pairs, so quit"<<endl;
		exit(0);
	}
	else cout<<"Good Matched points are " << good_matched_num <<endl;

	int i; 
	temp_ptr = good_items;
	for (i = 0; i < good_matched_num; i++)
	{
	...
	// set the parameters
	mwArray Coord_dest(2,good_matched_num, mxDOUBLE_CLASS);
	Coord_dest.SetData(dest, 2*good_matched_num);
	mwArray Coord_mimio(2,good_matched_num,mxDOUBLE_CLASS);
	Coord_mimio.SetData(mimio, 2*good_matched_num);
	mwArray result_matrix_inv;
	mwArray result_matrix;
	mwArray arrayNum(1, 1, mxINT32_CLASS);
	arrayNum.SetData(&good_matched_num, 1);
	int argNum = 2;

	matrix_value(argNum, result_matrix, result_matrix_inv,Coord_dest,Coord_mimio,arrayNum);
	
	double *p_matrix = new double [9];

	result_matrix.GetData(p_matrix, 8); // a bug here 4/16/06. 8 was 9.
	...
	result_matrix_inv.GetData(p_matrix, 9);
	for (i = 0; i < 9; i++)
	{
...
int item_seg(int y, int x, BYTE **image, BYTE **item_LUT, int& yMin, int& xMin, int& yMax, int& xMax, int& seg_pixels)
{
	...
	{
		cout<<"boundary problem in the item_seg"<<endl;
		return 0;
	}
	if (image[y-1][x] == 1) 
	{
