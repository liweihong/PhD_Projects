...
const int thresHough=185;		// for HT: icme06 185, ivr04 185, procams 190, good1 160, others: 120 150; 
const int thresExtract=5;		// for handwrting extraction: icme06 5, ivr04 5, procams 20 
...
void persptrans_image(BYTE** image, BYTE** image_pptnote, double* p_matrix, int w, int h, int newminx, int newminy, int neww, int newh);
void get_new_persptransform_image_size(int old_x[], int old_y[], int transformed_x[], int transformed_y[], double* p_matrix);
...
void camera_calibrate ( BYTE **&ppt_image,Bitmap *&ppt_bitmap )
{
	...
	BYTE **image_copy = new_image(w,h);
	BYTE **image_gray = new_image(w,h);
	
	Save2File("image\\gray.jpg",  oimage, NULL, &clsid, w, h);
	...
	for (x=0; x<w; x++)
	{
		image_copy[y][x] = image[y][x];
	}
	//get binary images from original image using the good threshold.
////	auto_thres_binary_image(image, w, h);
/**/
	binary_image(image, image1, thresHough, w, h);

	// threshold -20 is used for differing image background and handwrting.
	// an important parameter for notes extraction.
	binary_image(image, image,  thresHough - thresExtract, w, h);
...
Save2File("image\\bindary_image1.jpg",	image1,	NULL, &clsid, w, h);
...
//	Save2File("image\\logout1.jpg",	logout,	&clsid, w, h);
	...
	for (x = 0 ; x < w; x++)
		for (y = 0; y < h; y++)
			newimage[y][x] = newimage[y][x] | logout[y][x];
	Save2File("image\\loglineout.jpg", newimage, NULL, &clsid, w, h);
	

	// same threshold for hough transform?
	binary_image(image_note, image_note, thresHough, w, h);
...
// get the note from the image by subtract two original images.
	...
	mwArray Coord_source(4, 2, mxDOUBLE_CLASS);
	Coord_source.SetData(coord0, 8);
	mwArray Coord_target(4, 2, mxDOUBLE_CLASS);
	Coord_target.SetData(coord1, 8);
	mwArray B(8, 1, mxDOUBLE_CLASS);
	B.SetData(coord0, 8);
	mwArray R, R1;
	int argNum = 2;
	int tempInt = 4;
	mwArray arrayNum(1, 1, mxINT32_CLASS);
	arrayNum.SetData(&tempInt, 1);

	presp_transform(argNum, R, R1, Coord_source, Coord_target, B, arrayNum); 

	double* p_matrix = new double[8];
	R.GetData(p_matrix, 8);	

	double* p_matrix1 = new double[8];
	R1.GetData(p_matrix1, 8);	

	int transformed_x[4]={0,0,0,0}, transformed_y[4]={0,0,0,0};
	int old_x[4] = {0,w-1,w-1,0}, old_y[4]={0,0,h-1,h-1};
	get_new_persptransform_image_size(old_x, old_y, transformed_x, transformed_y, p_matrix1);

	int min_x = neww, min_y=newh, max_x=0, max_y=0;
	for (int i=0; i<4; i++)
	{
		if (transformed_x[i]<min_x)
			min_x = transformed_x[i];
		if (transformed_x[i]>max_x)
			max_x = transformed_x[i];
		if (transformed_y[i]<min_y)
			min_y = transformed_y[i];
		if (transformed_y[i]>max_y)
			max_y = transformed_y[i];
	}

	int image_overlap_ppt_video_w = max_x-min_x;
	int image_overlap_ppt_video_h = max_y-min_y;
	BYTE** image_ppt_video = new_image(image_overlap_ppt_video_w, image_overlap_ppt_video_h);
	
	Bitmap* bitmap_ppt_orig = new Bitmap(L"image\\ppt_1.jpg");

	for (y=0; y<image_overlap_ppt_video_h; y++)
	for (x=0; x<image_overlap_ppt_video_w; x++)
	{
		image_ppt_video[y][x] = 255;
	}
	persptrans_image(image_copy, image_ppt_video, p_matrix, w, h, min_x, min_y, max_x, max_y);
	Color i_color;
	Bitmap *ppt_video_bitmap = new Bitmap(image_overlap_ppt_video_w, image_overlap_ppt_video_h);
	for (y=0; y<image_overlap_ppt_video_h; y++)
	for (x=0; x<image_overlap_ppt_video_w; x++)
	{
		ppt_video_bitmap->SetPixel(x, y, Color(image_ppt_video[y][x], image_ppt_video[y][x],  image_ppt_video[y][x] ));
	}
	for (y=0; y<h; y++)
	for (x=0; x<w; x++)
	{
		bitmap_ppt_orig->GetPixel(x,y, &i_color);
		if (i_color.GetR()+i_color.GetG()+i_color.GetB()<600)
			ppt_video_bitmap->SetPixel(x-min_x, y-min_y, Color(i_color.GetR(), i_color.GetG(), i_color.GetB()));
	}

	Save2File("image\\overlap_ppt_video.jpg", NULL, ppt_video_bitmap, &clsid, image_overlap_ppt_video_w, image_overlap_ppt_video_h);
	...
	persptrans_image(image_onlynote, image_pptnote, p_matrix, w, h, 0, 0, neww, newh);
...
void get_new_persptransform_image_size(int old_x[], int old_y[], int transformed_x[], int transformed_y[], double* p_matrix)
{
	...
	for(int i=0; i<4; i++)
	{
		...
		transformed_x[i] = (int)floor(xx);
		transformed_y[i] = (int)floor(yy);
	}
}
...
void persptrans_image(BYTE** image, BYTE** image_pptnote, double* p_matrix, int w, int h, int newminx, int newminy, int neww, int newh)
{
	...
	int fx, fy; 
	for(int y=newminy; y<newh; y++)
	for(int x=newminx; x<neww; x++)
	{
		zz =  p_matrix[6]*x + p_matrix[7]*y + 1;
		xx = (p_matrix[0]*x + p_matrix[1]*y + p_matrix[2])/zz;
		yy = (p_matrix[3]*x + p_matrix[4]*y + p_matrix[5])/zz;
		...
		if(fx>0&&fx+1<w&&fy>0&&fy+1<h)
			image_pptnote[y-newminy][x-newminx]= (BYTE)((1-dy)*( (1-dx)*image[fy][fx]+dx*image[fy][fx+1] ) + dy*((1-dx)*image[fy+1][fx]+dx*image[fy+1][fx+1]));
	}
}
...
/**/
	...
	// set paramters.
	mwArray mangle(4, 1, mxDOUBLE_CLASS, mxREAL);
	mangle.SetData(angle4, 4);
	mwArray mdistance(4, 1, mxDOUBLE_CLASS, mxREAL);
	mdistance.SetData(distance4, 4);
	mwArray coordinate;
	int argNum = 1;
	double wD = w;
	double hD = h;
	mwArray arrayW(1, 1, mxDOUBLE_CLASS, mxREAL);
	arrayW.SetData(&wD, 1);
	mwArray arrayH(1, 1, mxDOUBLE_CLASS, mxREAL);
	arrayH.SetData(&hD, 1);

	get6points(argNum, coordinate, mangle, mdistance, arrayW, arrayH);

	double* result1 = new double[12]; // 12 - 6 points of (x,y)?
	coordinate.GetData(result1, 12);	// a bug here, when 12 was 8.
