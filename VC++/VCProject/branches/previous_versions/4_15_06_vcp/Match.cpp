...
const int NAVI_SIZE = 30;
const int VALID_P = 50;
const double MATCHED_RATIO = 0.6;
const int LEN = 120;
...
void matching(BYTE **ppt_image, BYTE **mimio_image, Bitmap *ppt_bitmap, Bitmap *mimio_bitmap, Bitmap *&matching, Bitmap *&matched, double *matrix_val)
{
	...
	for (int num = 0; num < item_num; num++)
	{
		...
		int x0, y0;
		int navi_size = diam > NAVI_SIZE ? NAVI_SIZE : diam;
	...
	if (good_matched_num < 4){
		cout<<"Sorry, there are only "<<good_matched_num<<" good matches, less than 4."<<endl
			<<"So I can't carry out the computation!"<<endl
			<<"Exit..."<<endl;
	}
