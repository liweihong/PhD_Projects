...
void item_seg(int y, int x, BYTE **image, BYTE **item_LUT, int& yMin, int& xMin, int& yMax, int& xMax);
...
void matching(BYTE **ppt_image, BYTE **mimio_image, Bitmap *ppt_bitmap, Bitmap *mimio_bitmap, Bitmap *&matching, Bitmap *&matched, double *matrix_val)
{
	...
	for (y=0; y<h; y++)
	{
		for(x=0; x<w; x++)
		{
			if (mimio_image[y][x] == BLACK && item_LUT[y][x] == 0)
			{
				...
				item_seg(y,x,mimio_image,item_LUT,yMin,xMin,yMax,xMax);
				items = new coord;
	...
	for (int num = 0; num < item_num; num++)
	{
		...
		if (((float)max_matched)/valide_pixels < 0.3) 
		{
			items_list = items_list->next;
...
void item_seg(int y, int x, BYTE **image, BYTE **item_LUT, int& yMin, int& xMin, int& yMax, int& xMax)
{
	...
	if (image[y-1][x] == 1) 
	{
		...
		item_seg(y-1,x,image,item_LUT,yMin,xMin,yMax,xMax);
	}
	if (image[y-1][x+1] == 1) 
	{
		if (yMin > y-1) yMin = y-1;
		if (xMax < x+1) xMax = x+1;
		item_seg(y-1,x+1,image,item_LUT,yMin,xMin,yMax,xMax);
	}
	if (image[y][x+1] == 1) 
	{
		if (xMax < x+1) xMax = x+1;
		item_seg(y,x+1,image,item_LUT,yMin,xMin,yMax,xMax);
	}
	if (image[y+1][x+1] == 1) 
	{
