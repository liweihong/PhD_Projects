...
void camera_calibrate ( BYTE **&ppt_image, Bitmap *&ppt_bitmap, BYTE **&pptprint_image, Bitmap *&pptprint_bitmap )
{
...
/**/
	binary_image(image, image1, 150, w, h);
	...
	convolve(image1, logout, 0, mask, 5, 5, w, h);
...
// get the note from the image by subtract two original images.
	...
	for (x=0; x<w; x++)
	{
		...
		if (image_onlynote[y][x]<50)
			image_onlynote[y][x]=0;	
		else image_onlynote[y][x]=255;	
//		if (image_onlynote[y][x]>200 && image1[y][x]<100)
//			image_onlynote[y][x]=0;			
	}

	Save2File("image\\binarynote_onlynote.bmp",	image_onlynote, NULL,&clsid, w, h);

	binary_image(image_note, image_note, 150, w, h);	
	Save2File("image\\undistortion.bmp",  image1, NULL, &clsid, w, h);
	Save2File("image\\undistortion_note.bmp",  image_note, NULL, &clsid, w, h);

	BYTE **image_pptprint = new_image(w,h);
	y=0, x=0;
	for (y=0; y<h; y++)
	{
		int minx=w-1, maxx=0; 
...
//				image_ppt[y][x] = 255;
		...
		for (x=minx; x<maxx; x++)
		{
			image_pptprint[y][x] = 255-image1[y][x];
		}
...
//				image_ppt[y][x] = 255;
	...
	Save2File("image\\undistortion1.bmp",  image_pptprint, NULL, &clsid, w, h);
	...
	//add 2/18
	pptprint_image = image_pptprint;
	pptprint_bitmap = new Bitmap(w, h);
	for ( i = 0; i < w; i++)
		for (int j = 0; j < h; j++)
			pptprint_bitmap->SetPixel(i,j, Color( 255 - pptprint_image[j][i],  255 - pptprint_image[j][i],  255 - pptprint_image[j][i] ));
