...
void scale_translate(BYTE **ppt_image, BYTE **mimio_image, Bitmap *ppt_bitmap, Bitmap *mimio_bitmap, Bitmap *&newbitmap, double *params)
{
	...
	int pixels_num = 0, pixels_total = 0;
	int yMin = 0, xMin = 0, yMax = 0, xMax = 0;

	// Set up the LUT for the connective component
	BYTE **item_LUT = new BYTE* [ppt_H];
	item_LUT[0] = new BYTE [ppt_H * ppt_W];
	for (int y=0; y<ppt_H; y++)
		item_LUT[y] = item_LUT[0] + y * ppt_W; 
	for (y=0; y < ppt_H; y++)
		for (int x=0; x<ppt_W; x++)
			item_LUT[y][x] = 0;
	...
	for (i = 0; i < 4; i ++)
	{
		...
		pixels_num = item_seg(corners1[i].y,corners1[i].x,ppt_image, item_LUT, yMin, xMin, yMax, xMax, pixels_num);
		pixels_total += pixels_num;
		scale1 += scales[i] * pixels_num;
		pixels_num = 0;
	}
	...
	int scaled_H = (int)(mimio_H * scale1) + abs(trans_y);
	int scaled_W = (int)(mimio_W * scale1) + abs(trans_x);
